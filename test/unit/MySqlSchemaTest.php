...
class MySqlSchemaTest extends TestCase
{
    ...
    {
        $this->assertEquals('mysql', $this->schema->driver);
    ...
    public function test_columns_returns_column_name_instance(): void
    {
        ...
        $this->assertInstanceOf(MySqlColumnName::class, $columnName);
    ...
    public function test_full_create_table_workflow(): void
    {
        $this->schema->create_table('users', true)
            ->columns()
                ->id_template()
                ->nextcolumnName('email')->varchar(255)->notnull()
            ->endcolumns()
            ->end();

        $this->assertStringContainsString('CREATE TABLE IF NOT EXISTS users', $this->schema->query);
        $this->assertStringContainsString('INTEGER PRIMARY KEY AUTO_INCREMENT', $this->schema->query);
        $this->assertStringContainsString('email VARCHAR(255)', $this->schema->query);
        $this->assertStringEndsWith(';', $this->schema->query);
    }
}
